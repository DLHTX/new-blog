    ...
    <div v-if="blogArr!=''">
        ...
        <div>
            <div class="blog_content">
                <router-link :to="{name:'blogDetail',query:{blogId:blog.blogId}}" class="blog_item animated pulse"
                    v-for="(blog,index) in blogArr" :key="index">
            ...
            <div style="height: 30px;">
                <el-pagination @current-change="handleCurrentChange" :current-page.sync="currentPage" :page-size="row"
...
export default {
    ...
    created() {
        if (localStorage.getItem('currentPage')) this.currentPage = Number(localStorage.getItem('currentPage'))
        this.getAllblog();
    },
    methods: {
        handleCurrentChange() {
        ...
        getAllblog: async function () {
            ...
            this.loading = true;
            try {
                localStorage.setItem('currentPage', this.currentPage)//currentPage存入本地
                let res = await blog.getBlog({ page: this.currentPage, row: this.row });
                this.loading = false;
                this.blogArr = ''
                this.blogArr = res.data;
                this.blogArr.forEach(item => {
                    item.src = item.bgImg ? item.bgImg : "https://uploadbeta.com/api/pictures/random/?key=BingEverydayWallpaperPicture&a=" + Math.random();
                    item.color = this.rgb();
